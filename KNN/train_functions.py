...
def reshape_img_matrix_single(img_8bit_matrix, photo_show):
    # 确保 img_8bit_matrix 的大小正确
    if img_8bit_matrix.shape[0] != 307200:
        raise ValueError("img_8bit_matrix must have shape (307200, 500)")

    # 检查是否传入有效的 photo_show 值
    if photo_show <= 0 or photo_show > img_8bit_matrix.shape[1]:
        raise ValueError("photo_show must be within the range of available images")
